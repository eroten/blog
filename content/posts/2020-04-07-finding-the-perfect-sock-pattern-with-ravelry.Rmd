...
dutchmasters::scale_fill_dutchmasters(palette = "pearl_earring") +
  labs(title = "Pattern Comment Density",
       x = "Comment count",
       y = "Density") +
  my_theme()
```
...
attributes_long %>% 
  ...
  geom_density(aes(x = difficulty_average),
               ...
               color = "gray",
               outline.type = "full") +
  facet_grid(reorder(pattern_attributes_permalink, 
                     difficulty_average, median)~.,
             switch = 'y', scales = "free_y") +
  scale_x_continuous(limits = c(2, 7)) +
  labs(title = 'Average difficulty rating by pattern attribute',
...
ggplot(project_rating_patterns) +
  geom_point(mapping = aes(x = difficulty_average,
                            y = rating_average,
                            color = free),
              size = 4) +
  dutchmasters::scale_color_dutchmasters(palette = "pearl_earring") +
  labs(title = "Average Rating and Average Difficulty",
...
project_rating_patterns %>% 
  ...
  arrange(desc(n)) %>% 
  ggplot() +
  geom_col(aes(x = pattern_attributes_permalink,
               y = n),
           fill = dutchmasters::dutchmasters$pearl_earring[9]) +
  labs(title = "Top pattern attributes",
       x = "Pattern attribute",
       y = "Frequency") +
  my_theme()
```
...
```{r, output.lines=1:8}
filter(project_rating_patterns, downloadable == TRUE,
...
```{r}
photo_links <- filter(project_rating_patterns, downloadable == TRUE,
       difficulty_average >= 4) %>% 
